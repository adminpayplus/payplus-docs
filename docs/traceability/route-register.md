...
| `BILLS-PAY` | `BILLS-ROOT` | Tab / view | Manage Saved/current Bills and Rent obligations the user needs or expects to pay; Bill payment entry consumes the applicable Tier 1/2/3 and Declaration handoff, while Rent retains mandatory Evidence. | DOC-06C | Defined baseline |
...
| `BILLS-ADD` | Bills / Pay+ | Flow / screen group | Create a controlled Bill, fee, Rent or tenancy obligation. Bill Evidence is optional for Tier 1 and required where the applicable Bill tier requires it; Rent retains mandatory attached Evidence. | DOC-06C | Defined baseline |
