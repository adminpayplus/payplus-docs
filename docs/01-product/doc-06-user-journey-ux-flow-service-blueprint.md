...
- ID establishment alone does not create a Saved/current, Saved/Archived or history-only projection. Saved/current, history-only and Saved/Archived are later projections of one source. An established source may remain unprojected only when immediate pay ends or fails before confirmed Payment, or deliberate Setup ends before its Saved/current projection is completed. DOC-09 owns applicable payment-lifecycle continuation/recovery, DOC-15 owns retention governance and requirements, and DOC-18 represents approved data/status/event/audit lineage and technical lifecycle facts. Save never creates a second ID, clones a source, or moves/recreates Payment lineage.
