...
| --- | --- | --- | --- | --- |
| Entrance and Authentication | DOC-06B | DOC-06A for journey; DOC-07/DOC-15/DOC-19 for disclosure, privacy, and security; DOC-20 for derived acceptance tests | Defined Behavior Baseline / Final Design and Child Mechanics Pending | `ENTRANCE-ROOT` is the public root; `AUTH-LOGIN` resolves to Fast/Full Login; Recovery, Registration, Account Activation, `PHONE-VERIFICATION`, `IDENTITY-VERIFICATION`, and `PAYMENT-PASSCODE-SETTINGS` destinations are assigned. Registration attempts reserve no identifiers; restricted account creation precedes phone, identity, and six-digit passcode completion through `ACCOUNT-ACTIVATION`. First-time identity verification does not require a pre-existing payment passcode. Normal success enters `HOME-ROOT`; approved deeplinks may resume their intended destination. |
...
| Approved prominent sensitive-value reveal and material changes to existing identity, contact, or Receiving Info data require payment passcode or approved reauthentication. First-time identity verification during Account Activation does not require a pre-existing payment passcode. Ordinary authenticated evidence, receipt, statement, invoice, and proof viewing/download does not require an extra prompt solely because the document is viewed or downloaded. | Confirmed |
| `ME-ROOT` is the permanent bottom-navigation account-control route for mixed-role users. DOC-06B defines Account Information, Phone Verification, reusable Identity Verification, Login & Security, Payment Passcode Settings route modes/handoffs, and Privacy & Data behavior while preserving handoffs to established feature owners. Detailed verification/passcode child-screen behavior remains pending. | Working Baseline / Core Account Routes Defined; Child Mechanics Pending |
...
- authentication acceptance includes `ENTRANCE-ROOT`, the `AUTH-LOGIN` family, `AUTH-RECOVERY`, `AUTH-REGISTRATION`, `ACCOUNT-ACTIVATION`, `PHONE-VERIFICATION`, `IDENTITY-VERIFICATION`, and `PAYMENT-PASSCODE-SETTINGS`, with normal successful entry to `HOME-ROOT`, approved contextual return where applicable, and detailed child-screen mechanics explicitly pending;
