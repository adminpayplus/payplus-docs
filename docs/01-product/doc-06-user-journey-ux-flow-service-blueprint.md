...
| --- | --- | --- | --- | --- |
| Entrance and Authentication | DOC-06B | DOC-06A for journey; DOC-07/DOC-15/DOC-19 for disclosure, privacy, and security | Defined Behavior Baseline / Final Design and Child Mechanics Pending | `ENTRANCE-ROOT` is the public root; `AUTH-LOGIN` resolves to Fast/Full Login; Recovery, Registration, Account Activation, and Phone Verification destinations are assigned. Registration attempts reserve no identifiers; restricted account creation precedes phone, identity, and six-digit passcode completion through `ACCOUNT-ACTIVATION`. Normal success enters `HOME-ROOT`; approved deeplinks may resume their intended destination. |
...
- authentication acceptance includes `ENTRANCE-ROOT`, the `AUTH-LOGIN` family, `AUTH-RECOVERY`, `AUTH-REGISTRATION`, and `ACCOUNT-ACTIVATION`, with normal successful entry to `HOME-ROOT` and approved contextual return where applicable;
