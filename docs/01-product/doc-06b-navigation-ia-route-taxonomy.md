...
| --- | --- | --- |
| 0.1.52 | 2026-08-06 | Changed the active definition status of `ENTRANCE-PROMOTION-DETAIL` to `Defined baseline` because its human-readable UI/UX behavior is decision-complete enough for continued alignment; preserved the accepted Carousel/detail/ownership meaning and kept final Copy, visual, accessibility, technical, operational, implementation, testing, acceptance, and validation evidence pending and non-blocking. |
