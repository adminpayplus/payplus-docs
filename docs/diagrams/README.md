...
| `payplus-app-route-entry-map.md` | Mermaid product-destination and navigation-transition map for bottom navigation, Pay+ actions, the eight dashboard shortcuts, and major route handoffs. It is a discussion reference and IA alignment aid, not final UI design. Destination identity/status is cross-checked against `docs/traceability/route-register.md`. | DOC-06B |
