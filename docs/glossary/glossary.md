...
| Phone Verification | The reusable `ACCOUNT-PROFILE` child flow that verifies control of the account's primary phone number. It is separate from identity verification and may be invoked contextually by Account Activation. | DOC-06B / DOC-15 |
| Identity Verification | The reusable `ACCOUNT-PROFILE` child flow for first-time identity verification and later permitted retry, correction, update, or re-verification. First-time verification during Account Activation does not require a pre-existing payment passcode; later changes require payment passcode or approved reauthentication. | DOC-06B / DOC-15 |
...
| Payment Passcode Settings | The reusable `ACCOUNT-SECURITY` child screen for Set, Change, or Reset and the permitted card/payment-profile confirmation preference. Account Activation may invoke Set contextually. | DOC-06B / DOC-19 |
