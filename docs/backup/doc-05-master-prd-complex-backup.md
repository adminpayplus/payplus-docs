...
Receive a payee-created payment request.
Review payment request details and evidence.
Accept, reject, query, dispute, or ignore a payee-created request as permitted by product rules.
Provide bill details or upload bill evidence where required.
Identify or select the payee or biller.
Receive a payment quote including service fee and total amount.
Confirm required disclosures and authorize payment.
Fund the transaction using supported payment methods.
Track verification, funding, payout, settlement, refund, and exception states.
Receive receipts and communications.
Access transaction history and support.
The product should allow an onboarded or approved payee to:
Create a payee profile or payee account.
Complete required onboarding, verification, and eligibility checks.
Create a bill, invoice, fee, rent, or approved payment obligation request.
Attach required evidence.
Identify the payer through approved identifiers or invitation mechanisms.
Send the payment request to the payer.
Track request status at an appropriate and privacy-safe level.
Receive payout only after payer authorization, successful funding, payee verification, payout readiness, and applicable controls.
Access payee-side receipts, remittance details, payout status, and support where approved.
The product should allow internal teams to:
Review bills, invoices, rent requests, fee requests, and supporting evidence.
Review onboarded payees, landlords, billers, or service providers.
Manage verification exceptions.
Monitor payer-payee relationship risk and collusion risk.
Approve, reject, or escalate payment requests.
Track payout status.
Resolve payout, refund, chargeback, and reconciliation exceptions.
Retrieve evidence for compliance, audit, partner review, and support.
---
...
User completes required profile and eligibility information.
User starts a bill payment request.
User selects bill category.
User enters bill details and uploads bill evidence.
System extracts or validates bill information.
System captures payee or biller details.
System verifies eligibility, bill evidence, payee, limits, and risk conditions.
User receives quote showing bill amount, service fee, taxes if applicable, total charge, timing, and refund/cancellation terms.
User accepts disclosures and authorizes payment.
User completes card payment through approved PSP/acquirer flow.
System records payment authorization and funding status.
System performs final risk and payout readiness checks.
System initiates or queues payout to approved payee.
User receives payment status updates and receipt.
Finance and operations reconcile the transaction.
Records are retained.
11.2 Payee Onboarding Journey
Payee creates account or is invited to PayPlus.
Payee selects or is assigned a payee type, such as landlord, biller, school, utility provider, service provider, or other approved type.
Payee provides required profile, business, identity, contact, payout, tax, or verification information based on payee type and launch rules.
Payee provides required supporting documentation where applicable.
System performs sanctions, eligibility, duplicate, risk, and payout destination checks.
Internal reviewer approves, rejects, requests additional information, or escalates the payee onboarding case where required.
Approved payee receives permitted capabilities based on payee type, category permissions, risk tier, and product configuration.
Payee actions and onboarding evidence are retained.
11.3 Payee-Created Bill, Invoice, Fee, or Rent Request Journey
Approved payee signs in.
Payee selects create payment request.
Payee selects permitted request category, such as rent, invoice, fee, bill, or other approved payment obligation.
Payee enters required request details:
...
Supporting evidence.
For rent, payee uploads or references approved tenancy or lease evidence where required.
System validates category eligibility, payee permissions, request completeness, evidence sufficiency, amount limits, duplicate signals, and risk rules.
Request is either:
Sent to payer.
Routed to manual review before being sent.
Rejected.
Returned to payee for more information.
Payer receives notification or invitation to review the request.
Payer reviews payee identity, request details, amount, evidence availability where applicable, service fee, total charge, timing, refund/cancellation rules, and PayPlus role.
Payer accepts and authorizes payment, rejects the request, raises a query, disputes the request, or allows it to expire.
If payer accepts, payment proceeds through approved funding flow.
Payout proceeds only after funding success, payee verification, risk checks, and payout readiness conditions are satisfied.
Payee and payer receive appropriate status updates.
Finance and operations reconcile the transaction.
Records are retained.
11.4 Returning User Journey
User signs in.
User may reuse eligible profile details, payment profile, payee, prior biller context, or prior payer/payee relationship where permitted.
User creates a new bill payment request or receives a payee-created request.
System applies current category, risk, limit, disclosure, fee, and partner rules.
User completes payment and tracks status.
11.5 Manual Review Journey
System identifies review trigger.
Request is routed to appropriate review queue.
Reviewer views user, payer, payee, bill, invoice, rent evidence, payment, risk, and history evidence.
Reviewer approves, rejects, requests more information, or escalates.
Decision is logged with reason and reviewer identity.
User, payer, or payee receives appropriate status communication.
Payout proceeds only if required approvals are complete.
11.6 Failed Payment Journey
User attempts payment.
Authorization fails, authentication fails, or PSP returns decline/error.
System communicates failure reason at an appropriate level.
User may retry with same or different supported funding source, subject to velocity and risk rules.
System prevents duplicate funding and maintains accurate request status.
If the request was payee-created, the payee sees only appropriate request status and must not receive sensitive payer funding failure details unless approved.
11.7 Payout Failure Journey
Funding succeeds and payout is initiated or queued.
Payout fails, is returned, delayed, or requires investigation.
System creates operations exception.
User and payee receive appropriate communication without misleading guarantee.
Operations resolves by retry, correcting payee details, refund, escalation, or other approved action.
Reconciliation and ledger records are updated.
11.8 Refund or Cancellation Journey
User, payee, or operations requests cancellation or refund, where permitted.
System determines transaction state and eligibility.
System applies approved refund/cancellation rules.
System reverses or refunds funding source where possible.
System handles promotion reversal where applicable.
System updates ledger, receipt, notifications, payer/payee status, and support records.
If a payee-created request is cancelled before payer authorization, no funding reversal is needed, but the request lifecycle and audit trail must be retained.
11.9 Dispute or Chargeback Journey
Chargeback or dispute is received from PSP/acquirer, payer, payee, or support channel.
System or operations links dispute to transaction, user, bill, invoice, rent evidence, payee, payer authorization, receipt, and payout evidence.
Operations prepares evidence package.
Risk and finance assess account, loss, and recovery actions.
Outcome is logged and ledger/reporting records are updated.
---
