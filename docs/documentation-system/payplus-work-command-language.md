...
| **Draft** | Create or revise accepted content in the primary owner. Editing is limited to the approved scope. | Accepted decision or exact edit scope, primary owner, writable files, material-work coverage matrix | Primary-owner draft, decision-to-representation coverage, assumptions and open items, draft-review handoff | Stages 7-9 | Stop for unresolved material decisions; otherwise at the Draft Review gate | May be used when authorized; one canonical writer controls each formal document |
| **Review** | Examine sources, a draft, or a diff and report findings. Read-only unless a separate Edit command or corrective scope is authorized. | Review target, review lens, authority sources, named findings when verifying | Phase-bounded findings with severity, evidence, ownership, recommendations, unresolved items, and canonical review result | Stage 4 or 9; may support Stage 12 when explicitly scoped as validation review | Primary Review may find any evidence-backed defect; Verification and Final Verification use the canonical convergence limits | May be recommended or used when authorized to improve independence or coverage |
...
| **Align** | Synchronize materially affected owners and references after the primary change. Do not create new requirements. | Accepted primary change, Change Impact Manifest or affected-document set | Owner-first cross-document updates, unresolved conflicts, and `ALIGN_EXECUTED - PENDING_VALIDATE` | Stages 10-11 | Stop on new material decisions, ownership conflict, or scope expansion; never declare coordinated success | May be used when authorized for disjoint files or review; Integration Lead consolidates |
| **Validate** | Test the coordinated result against scope, ownership, consistency, links, format, and applicable acceptance criteria. Read-only except for separately authorized corrections. | Intended scope, changed files, applicable checks | `VALIDATE_FAIL - RETURN_TO_<STAGE>` or `REVALIDATE_PASS - READY_TO_INTEGRATE`, with evidence and unresolved items | Stages 12-13 | Failed checks block integration readiness; Validate alone owns coordinated gate pass/fail and does not authorize Commit | May be used when authorized for specialist checks; canonical validation conclusion remains with the workflow |
| **Integrate** | Complete impact analysis, Align affected artifacts, Validate the coordinated result, and prepare the pre-commit report. | Accepted primary change, writable scope, repository baseline | Integrated diff, validation evidence, open issues, pre-commit report, and canonical integration result | Stages 10-13 | Report `INTEGRATE_PASS - READY_FOR_COMMIT_APPROVAL` only after validation passes; otherwise return to the named stage | May be used when authorized; the Integration Lead owns consolidation |
...
11. Use the canonical stage-result vocabulary from the Documentation Development Workflow. Do not use `PASS WITH MAJOR CORRECTIONS` or equivalent mixed results.
...
material alternatives, run the bounded challenge and Decision Readiness checks,
and return one consolidated material Founder Decision Pack wherever practical
with an exact edit boundary. Read-only. Stop for my Founder Decision.
```
...
affected references and Validate the coordinated result. For material work, use
the decision coverage matrix. Preserve unrelated changes. Stop with the canonical
validation result and pre-commit report. Do not Commit or Push.
```
