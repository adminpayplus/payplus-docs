...
| Provider API, callback, query, file, partner integration, or provider-environment contract | DOC-17 for the reviewed provider-neutral contract and any later separately accepted provider-specific contract; DOC-16 and the current domain owner remain authoritative for architecture and domain boundaries | DOC-18 data/event representation and DOC-19 security treatment where applicable |
